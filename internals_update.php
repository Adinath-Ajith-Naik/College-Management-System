...
if($_SERVER['REQUEST_METHOD'] == "POST")
	{
		...
		if(!empty($student_name) && !empty($mark1) && !empty($mark2))
		{
			...
			//save to database
			$query = "update internals set marks1=".$mark1.", marks2=".$mark2." where user_name ='".$student_name."' and subjects='".$user_data['faculty_subject']."'";
                        ...
                        <div class="flex w-full justify-center items-end">
                            <button
                                class="flex mx-auto mb-10 text-white bg-gray-800 border-0 py-2 px-8 focus:outline-none hover:bg-gray-700 rounded text-lg"><a
                                    href="internal_faculty.php">Go Back</a></button>
                        </div>
                    </div>
                </div>
            </div>
