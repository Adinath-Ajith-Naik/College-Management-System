...
if($_SERVER['REQUEST_METHOD'] == "POST")
	{
		...
		if(!empty($student_name) && !empty($attendance))
		{
            echo "enterd IF Condition";
			//save to database
			$query = "update attendance set attendance=".$attendance." where user_name ='".$student_name."' and subjects='".$user_data['faculty_subject']."'";

			mysqli_query($con, $query);

			header("Location: attendance_faculty.php");
			die;
		}else
		{
        ...
        <form method="POST">
            <div class="container px-5 py-24 mx-auto">
                <div class="flex flex-col text-center w-full mb-12">
                    <h1 class="sm:text-3xl text-2xl font-medium title-font mb-4 text-white">Update Attendance </h1>
                </div>
                    ...
                    <div class="flex flex-wrap -m-2">
                        ...
                        <div class="flex w-full justify-center items-end">
                            <div class="relative mr-4 my-4 lg:w-full align-middle xl:w-1/2 w-2/4 md:w-full text-left">
                                <label for="hero-field" class="leading-7 text-sm text-gray-400">Attendance</label>
                                <input type="number" id="topic" name="attendance"
                                    class="w-full bg-white rounded border bg-opacity-40 border-gray-700 focus:ring-2 focus:ring-red-900 focus:bg-transparent focus:border-red-500 text-base outline-none text-gray-100 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                            </div>
                        </div>
                        <div class="p-2 w-full mt-4">
                            <button onclick=""
                                class="flex mx-auto text-white bg-red-500 border-0 py-2 px-8 focus:outline-none hover:bg-gray-600 rounded text-lg">Publish
                                Attendance</button>
                        </div>
                        <div class="flex w-full justify-center items-end">
            <button
                class="flex mx-auto mb-10 text-white bg-gray-800 border-0 py-2 px-8 focus:outline-none hover:bg-gray-700 rounded text-lg"><a
                    href="attendance_faculty.php">Go Back</a></button>
        </div>
                    </div>
                </div>
            </div>
